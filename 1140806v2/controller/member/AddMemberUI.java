...
public class AddMemberUI extends JFrame {
	...
	public AddMemberUI() {
		...
		message.setBounds(213, 106, 213, 43);
		...
		creatMember.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				String custMemberName=(memberName.getText()!=null&&!memberName.getText().trim().isEmpty())?memberName.getText():null;
				String custMemberUsername=(memberUsername.getText()!=null&&!memberUsername.getText().trim().isEmpty())?memberUsername.getText():null;
				String custMemberPassword=(memberPassword.getText()!=null&&!memberPassword.getText().trim().isEmpty())?memberPassword.getText():null;
				String custMemberPhone=(memberPhone.getText()!=null&&!memberPhone.getText().trim().isEmpty())?memberPhone.getText():null;
				if(custMemberName==null)message.setText("請輸入姓名");
				if(custMemberUsername==null)message.setText("請輸入帳號");
				if(custMemberPassword==null)message.setText("請輸入密碼");
				if(custMemberPhone==null)message.setText("請輸入電話");
				CustMember custMember=new CustMember(custMemberName,custMemberUsername,custMemberPassword,custMemberPhone);
				if(custMember!=null)
				{
					new CustMemberServiceImpl().addCustMember(custMember);
		...
		creatMember.setBounds(268, 159, 100, 23);
		...
		clear.setBounds(268, 192, 100, 23);
		...
		leave.setBounds(268, 223, 100, 23);
